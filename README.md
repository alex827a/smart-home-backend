...
**Solutions:**
- ✅ Verify Mosquitto is running: `Get-Process mosquitto`
- ✅ Check port is listening: `netstat -an | findstr 8883`
- ✅ Test with mosquitto_sub (see Testing section)
...
**Solutions:**
- ✅ Install CA: `mkcert -install`
- ✅ Verify CA path in `MQTT_CA_FILE`
...
**Solutions:**
- ✅ Verify password file: `C:\mosquitto\passwd`
- ✅ Check ACL permissions: `C:\mosquitto\acl`
- ✅ Ensure `allow_anonymous false` in config
- ✅ Restart Mosquitto after ACL changes

#### 4. Module Not Found: aiomqtt

**Symptoms:**
```
ImportError: No module named 'aiomqtt'
```
...
#### 5. Windows Event Loop Error

**Symptoms:**
```
NotImplementedError: Cannot run the event loop while another loop is running
```

**Solutions:**
- ✅ Use `run_server.py` instead of `uvicorn server:app` directly
- ✅ Ensure `WindowsSelectorEventLoopPolicy` is set (already in code)

### Debug Mode

Enable verbose MQTT logging in Mosquitto:
...
Check server logs for MQTT publish confirmations:
```
MQTT connected to 127.0.0.1:8883 (tls=True)
MQTT: Message published to home/system/metrics
```
...
- Test with both TLS and non-TLS modes
...
- **SmartHome MAUI Client**: [Link to client repo]
